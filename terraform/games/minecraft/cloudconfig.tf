...
data "template_file" "minecraft" {
  template = <<-EOF
    ...
    runcmd:
      ...
      - docker run --name set_route -e AWS_DEFAULT_REGION=${var.aws_region} -e FQDN=${var.subdomain}.${replace(data.aws_route53_zone.zone.name, "/[.]$/", "")} -e ZONE_ID=${var.hosted_zone_id} -e GAME=minecraft ${var.tools_docker_image_id} set_route.py
